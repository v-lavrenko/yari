...
END {
	...
	fmt = "%9d: %7.3f ± %5.3f [%6.2f %6.2f %6.2f %6.2f %6.2f |%6.2f| %6.2f %6.2f %6.2f %6.2f %6.2f] %8s\n";
	hmt = "%9s: %7s ± %5s [%6s %6s %6s %6s %6s |%6s| %6s %6s %6s %6s %6s] %4s\n";
    }
    if (org) { # table for org-mode
	gsub("[][]","",fmt); gsub("[:|±]","",fmt); gsub("  *","|",fmt); fmt="|"fmt;
	gsub("[][]","",hmt); gsub("[:|±]","",hmt); gsub("  *","|",hmt); hmt="|"hmt;
    }
    if (H) printf hmt, "N", "mean", "std.err", "min", "1%", "5%", "10%", "25%", "medi", "75%", "90%", "95%", "99%", "max", "file";
    if (N) printf fmt, N, mean, sterr, min, pct01, pct05, pct10, pct25, pct50, pct75, pct90, pct95, pct99, max, (tag?tag:FILENAME);
    #printf "Count:    %d\n", count;
    #printf "Mean:     %g\n", avg;
    #printf "Variance: %g\n", var;
    #printf "St.dev:   %g\n", var^0.5;
